...
class PentaQLoss(Function):

    def __init__(self, margin, num_classes=10):
        super(PentaQLoss, self).__init__()
        self.margin = margin
        self.num_classes = num_classes
        self.pdist = PairwiseDistance(2)  # norm 2

    def forward(self, anchor, positive, negative, anc_mea, neg_mea):
        d_p = self.pdist.forward(anchor, positive)
        d_n = self.pdist.forward(anchor, negative)
        d_m = self.pdist.forward(anc_mea, neg_mea)
        dist_hinge = torch.clamp(self.margin + d_p - d_n, min=0.0) + \
                     torch.clamp(self.margin*2 - d_m, min=0.0)
        loss = torch.mean(dist_hinge)
        return loss
...
class PairwiseDistance(Function):
    ...
    def forward_val(self, x1, x2):
        eps = 1e-5
        diff = torch.abs(x1 - x2)
        out = torch.pow(diff, self.norm).sum(dim=0)
        # print(out)
        return torch.pow(out + eps, 1. / self.norm)

def generate_csv(train_set, train_set_csv ):
    c = open(train_set_csv, 'w+')
    c.write('id' + ',' + 'name' + ',' + 'class' + '\n')
    for p, d, f in os.walk(train_set):
        i = 0
        for dir in d:
...
def select_triplet_val(dir, num_classes, limit=20, is_car=False):
    '''
    从triplet_dir中随机选择一个三元组
    '''
    # np.random.seed(100)  # 设置固定的随机数种子,便于验证
...
def get_all_hard_triplets(dir, model_path,
    ...
    del model.forward_classifier
    model.forward_classifier = lambda x: x
    model.cuda()  # 模型放进GPU
    model.eval()  # 求值模式
    '''
    # 训练数据集所有文件名
    ...
    for anchor_cls in tqdm(range(num_classes)):
        ...
        with torch.no_grad():
            for id in range(num_per_cls):
                ...
                anchor_img = Variable(transform(Image.open(anchor_path)).unsqueeze(0)).cuda()  # 数据放入GPU
                ...
                for nega_cls in range(num_classes):# 遍历每一个负样本类别
                    ...
                    for nega_id in range(nega_start, nega_start + num_per_cls):
                        ...
                        nega_img = Variable(transform(Image.open(nega_path)).unsqueeze(0)).cuda()  # 数据放入GPU
